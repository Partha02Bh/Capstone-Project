...
// import com.example.demo.service.RewardService;                   // NEW
...
//             logger.warn("Withdrawal failed - insufficient funds: userId={}, balance={}, requested={}",
...
//         logger.info("Withdrawal successful: userId={}, amount={}, newBalance={}", userId, amount, account.getBalance());
//         return ResponseEntity.ok("Withdrawal Successful");
//     }

//     // ── TRANSFER (with Reward Module integration) ─────────────────────────────
...
//         logger.info("Transfer request: sourceUserId={}, targetUserId={}, amount={}",
//                 sourceUserId, targetUserId, amount);

//         // Rule 4: Not self-transfer (also covers Rule 3: different users)
...
//                 .orElseThrow(() -> {
//                     logTransaction(sourceAccount.getId(), amount.negate(), "TRANSFER_OUT",
//                             null, "FAILED", "Receiver Account not found");
...
//             logTransaction(sourceAccount.getId(), amount.negate(), "TRANSFER_OUT",
//                     targetAccount.getId(), "FAILED", "Insufficient Funds");
//             throw new InsufficientFundsException();
//         }
...
//         // Log TRANSFER_OUT (returns saved log so we can link the reward to it)  // CHANGED
//         TransactionLog sourceLog = logTransactionAndReturn(                        // CHANGED
//                 sourceAccount.getId(), amount.negate(), "TRANSFER_OUT",
//                 targetAccount.getId(), "SUCCESS", null);

//         logTransaction(targetAccount.getId(), amount, "TRANSFER_IN",
//                 sourceAccount.getId(), "SUCCESS", null);

//         logger.info("Transfer successful: from userId={} to userId={}, amount={}",
//                 sourceUserId, targetUserId, amount);

//         // ── REWARD MODULE ─────────────────────────────────────────────────────
//         // Rule 1: status=SUCCESS ✓ (we only reach here on success path)
//         // Rule 2: amount > ₹100  — checked inside RewardService.isEligible()
//         // Rule 3: different users ✓ (SelfTransferException thrown above if same)
//         // Rule 4: not self-transfer ✓ (same check)
//         rewardService.processReward(
//                 sourceAccount.getUser().getId(),   // sender's user ID
//                 sourceAccount.getId(),             // sender's account ID
//                 sourceLog.getId(),                 // linked transaction log ID
//                 amount                             // positive amount for calculation
//         );
//         // ─────────────────────────────────────────────────────────────────────
...
//     /** Original helper — still used for deposit, withdraw, TRANSFER_IN */
//     private void logTransaction(Long accountId, BigDecimal amount, String type,
//                                  Long relatedAccountId, String status, String reason) {
//         logTransactionAndReturn(accountId, amount, type, relatedAccountId, status, reason);
//     }

//     /**
//      * NEW — same as logTransaction but returns the saved entity.
//      * Used for TRANSFER_OUT so we can link the reward to the transaction ID.
//      */
//     private TransactionLog logTransactionAndReturn(Long accountId, BigDecimal amount,
//                                                     String type, Long relatedAccountId,
...
//         TransactionLog saved = transactionLogRepo.save(log);
//         logger.info("Transaction logged: type={}, amount={}, status={}, accountId={}",
//                 type, amount, status, accountId);
//         return saved;
//     }
// }
...
public class TransactionController {
    ...
    public ResponseEntity<?> transfer(@RequestBody Map<String, String> request) {
        ...
        Long targetUserId = Long.parseLong(request.get("targetId"));
        BigDecimal amount = new BigDecimal(request.get("amount"));
        String idempotencyKey = request.get("idempotencyKey"); // ── IDEMPOTENCY ──

        logger.info("Transfer request: sourceUserId={}, targetUserId={}, amount={}, key={}",
                sourceUserId, targetUserId, amount, idempotencyKey);

        // ── IDEMPOTENCY CHECK — reject duplicate submissions ──────────────────
        if (idempotencyKey != null && !idempotencyKey.isBlank()) {
            transactionLogRepo.findByIdempotencyKey(idempotencyKey).ifPresent(existing -> {
                logger.warn("Duplicate transfer blocked: key={}", idempotencyKey);
        ...
        // Log TRANSFER_OUT (returns saved log so we can link the reward to it)
        TransactionLog sourceLog = logTransactionAndReturn(
                sourceAccount.getId(), amount.negate(), "TRANSFER_OUT",
                targetAccount.getId(), "SUCCESS", null, idempotencyKey); // ── save key ──
    ...
    /**
     * Original helper — deposit, withdraw, TRANSFER_IN (no idempotency key needed)
     */
    private void logTransaction(Long accountId, BigDecimal amount, String type,
            Long relatedAccountId, String status, String reason) {
        logTransactionAndReturn(accountId, amount, type, relatedAccountId, status, reason, null);
    }

    /**
     * Returns the saved entity — used for TRANSFER_OUT so we can:
     * 1. Link the reward to the transaction ID
     * 2. Save the idempotency key to prevent duplicate submissions
     */
    private TransactionLog logTransactionAndReturn(Long accountId, BigDecimal amount,
            String type, Long relatedAccountId,
            String status, String reason,
        ...
        log.setStatus(status);
        log.setReasonCode(reason);
        log.setIdempotencyKey(idempotencyKey); // ── save key ──

        TransactionLog saved = transactionLogRepo.save(log);
        logger.info("Transaction logged: type={}, amount={}, status={}, accountId={}, key={}",
                type, amount, status, accountId, idempotencyKey);
        return saved;
    }
}